...
if __name__ == "__main__":
    ...
    # ==================
    # prior converged fit from jointfit_all.py -- every line's pkl there
    # holds the same full compound model, so any one of them gives every
    # component's previously-fit mean/stddev/amplitude. Used below as the
    # anchor/source of the tighter bounds (any degeneracy between
    # overlapping components already resolved once by that fit).
    # ==================
    with open('./output/joint_fit/jointfit_all/Halpha_joint_tied_fit.pkl',
              'rb') as f:
        prior_model = dill.load(f)['model']
    ...
    for name in line_order + ['[NII]6584']:
        ...
        ax[0].set_title(f"{name}  |  extra-constrained joint tied fit (all lines tied to Halpha)  |  z_A={z_A}  z_B={z_B}",
                        fontsize=12)
        ...
        fig.savefig(
            f'./output/joint_fit/extra_constrained/{name}_joint_tied_fit.png')
        with open(
                f'./output/joint_fit/extra_constrained/{name}_joint_tied_fit.pkl',
                'wb') as f:
