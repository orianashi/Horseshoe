...
# ==================
# master components (free): Halpha's 4 components (A: central, red wing;
# B: core [merged red+central], blue wing). Everything else ties its
# mean/stddev to these.
#
# Source B previously had 3 free Halpha components (red, central, blue), but
# the joint fit's own covariance matrix showed red/central amplitudes
# correlated at -0.937 -- essentially degenerate, the fitter could trade flux
# between them almost for free without changing the total model. blue was
# only weakly correlated with either (~0.29-0.31). This file merges red+central
# into one new free 'core' component (its own mean/stddev/amplitude, not
# preset to either old role's guess) and keeps blue separate, to test whether
# removing that specific degeneracy improves the fit. See jointfit_all.py for
# the original 3-component-B version; both are kept so results can be compared
# (compare_twocomponent_B.py does exactly that).
# ==================
master_guesses = {
    'Halpha_A_central': {
        'z_guess': z_A,
    ...
    'Halpha_B_core': {
        # merged replacement for the old Halpha_B_red + Halpha_B_central,
        # deliberately using bounds wide enough to not bias the fit toward
        # either old role: z_guess is the midpoint of the old z_B+0.0006
        # (red) and z_B (central) guesses; amplitude/stddev guesses are the
        # average of the old two; stddev_bounds and mean_range are the union
        # (widened, not narrowed) of the old two components' bounds.
        'z_guess': z_B + 0.0003,
        'amplitude': 6,
        'stddev': 1.8,
        'stddev_bounds': (0, 5),
...
colors = {
    ...
    ('B', 'red'): 'red',
    ('B', 'blue'): 'blue',
    # new merged Source B category (replaces the old red/central pair) --
    # deliberately distinct from every old color so it reads as "new merged
    # component", not either old role
    ('B', 'core'): 'green',
}
...
if __name__ == "__main__":
    ...
    # above, and its Gaussian components are tied to and fit jointly with
    # Halpha there. Adding it to `line_order` would double-count those pixels
    # and add a second, unconstrained continuum term overlapping Halpha's.
    ...
    # Source B has 2 components per line (core [merged red+central], blue)
    # instead of jointfit_all.py's 3 (red, central, blue) -- every old ref=2
    # (B_red) or ref=3 (B_central) becomes ref=2 (B_core); every old ref=4
    # (B_blue) becomes ref=3. [OII]'s B-side collapses from 2 components
    # (red+central) to 1 (core) since both its old tie targets merged into
    # one; [NII]6584_B simply re-points from ref 3 to ref 2.
    #  0: Halpha_A_central   1: Halpha_A_red
    #  2: Halpha_B_core      3: Halpha_B_blue
    #  4: [OIII]5007_A_central (tied 0)   5: [OIII]5007_A_red (tied 1)
    #  6: [OIII]5007_B_core (tied 2)      7: [OIII]5007_B_blue (tied 3)
    #  8: [OIII]4959_A_central (tied 0)   9: [OIII]4959_A_red (tied 1)
    # 10: [OIII]4959_B_core (tied 2)     11: [OIII]4959_B_blue (tied 3)
    # 12: Hbeta_A_central (tied 0)       13: Hbeta_A_red (tied 1)
    # 14: Hbeta_B_core (tied 2)          15: Hbeta_B_blue (tied 3)
    # 16: Hgamma_A_central (tied 0)      17: Hgamma_A_red (tied 1)
    # 18: Hgamma_B_core (tied 2)         19: Hgamma_B_blue (tied 3)
    # 20: [OII]3726_A (tied 0, wing amplitude bounds)
    # 21: [OII]3726_B_core (tied 2)
    # 22: [OII]3729_A (tied 0)
    # 23: [OII]3729_B_core (tied 2, wing amplitude bounds)
    # 24: [NII]6584_A (tied 0, single component -- no wing decomposition)
    # 25: [NII]6584_B (tied 2, single component -- no wing decomposition)
    # 26: continuum_Halpha  27: continuum_[OIII]5007  28: continuum_[OIII]4959
    # 29: continuum_Hbeta   30: continuum_Hgamma      31: continuum_[OII]
    # ==================
    gaussians = []

    gaussians.append(build_master('Halpha_A_central', rest['Halpha']))  # 0
    gaussians.append(build_master('Halpha_A_red', rest['Halpha']))  # 1
    gaussians.append(build_master('Halpha_B_core', rest['Halpha']))  # 2
    gaussians.append(build_master('Halpha_B_blue', rest['Halpha']))  # 3

    r_OIII5_ha = rest['[OIII]5007'] / rest['Halpha']
    gaussians.append(
        build_tied('[OIII]5007_A_central', 0, r_OIII5_ha, 50, (0, None)))  # 4
    gaussians.append(build_tied('[OIII]5007_A_red', 1, r_OIII5_ha, 10, (0, None)))  # 5
    gaussians.append(
        build_tied('[OIII]5007_B_core', 2, r_OIII5_ha, 3.5, (0, None)))  # 6
    gaussians.append(build_tied('[OIII]5007_B_blue', 3, r_OIII5_ha, 1, (0, None)))  # 7

    r_OIII4_ha = rest['[OIII]4959'] / rest['Halpha']
    gaussians.append(
        build_tied('[OIII]4959_A_central', 0, r_OIII4_ha, 50, (0, None)))  # 8
    gaussians.append(build_tied('[OIII]4959_A_red', 1, r_OIII4_ha, 10, (0, None)))  # 9
    gaussians.append(
        build_tied('[OIII]4959_B_core', 2, r_OIII4_ha, 3.5, (0, None)))  # 10
    gaussians.append(build_tied('[OIII]4959_B_blue', 3, r_OIII4_ha, 1, (0, None)))  # 11

    r_hb_ha = rest['Hbeta'] / rest['Halpha']
    gaussians.append(
        build_tied('Hbeta_A_central', 0, r_hb_ha, 50, (0, None)))  # 12
    gaussians.append(build_tied('Hbeta_A_red', 1, r_hb_ha, 10, (0, None)))  # 13
    gaussians.append(
        build_tied('Hbeta_B_core', 2, r_hb_ha, 3.5, (0, None)))  # 14
    gaussians.append(build_tied('Hbeta_B_blue', 3, r_hb_ha, 1, (0, None)))  # 15

    r_hg_ha = rest['Hgamma'] / rest['Halpha']
    gaussians.append(
        build_tied('Hgamma_A_central', 0, r_hg_ha, 3, (0, None)))  # 16
    gaussians.append(build_tied('Hgamma_A_red', 1, r_hg_ha, 1, (0, None)))  # 17
    gaussians.append(
        build_tied('Hgamma_B_core', 2, r_hg_ha, 3.5, (0, None)))  # 18
    gaussians.append(build_tied('Hgamma_B_blue', 3, r_hg_ha, 1, (0, None)))  # 19
    ...
    # Source B now has a single merged 'core' component (ref_idx 2), replacing
    # the separate red+central components jointfit_all.py used -- forced by
    # the Halpha-side merge (both old tie targets collapsed into one), and it
    # mirrors the structure [OII]_A already had (single, role-less component).
    # [OII]3726_A and [OII]3729_B_core are blended with each other, so their
    # amplitude bounds replicate oii_gaussian_fitting.py lines 220-233,
    # derived from the unblended wing fit's amplitudes -- the bound on the old
    # B_central carries over unchanged onto the merged B_core since it
    # constrains total blended flux in the region, independent of how many
    # kinematic components carry it.
    r_oii3726_ha = rest['[OII]3726'] / rest['Halpha']
    gaussians.append(
        build_tied('[OII]3726_A', 0, r_oii3726_ha, amp0_wing / 1.4,
                   (amp0_wing / 1.5, amp0_wing / 0.35)))  # 20
    gaussians.append(
        build_tied('[OII]3726_B_core', 2, r_oii3726_ha, 1 + amp1_wing, (0, None)))  # 21

    r_oii3729_ha = rest['[OII]3729'] / rest['Halpha']
    gaussians.append(
        build_tied('[OII]3729_A', 0, r_oii3729_ha, amp0_wing, (0, None)))  # 22
    # guess kept as the old B_central-alone value (not summed with the old
    # B_red guess of 1): the inherited bound (amp0_wing*0.35, amp1_wing*1.5)
    # was calibrated for B_central alone, and amp1_wing*1.4 + 1 falls outside
    # its upper edge, which `least_squares` rejects as an infeasible x0.
    gaussians.append(
        build_tied('[OII]3729_B_core', 2, r_oii3729_ha, amp1_wing * 1.4,
                   (amp0_wing * 0.35, amp1_wing * 1.5)))  # 23

    # [NII]6584: single Gaussian per source (no red/blue wing decomposition,
    # unlike Halpha/OIII/Hbeta/Hgamma), tied to Halpha's central/core
    # component. Amplitude guesses scaled down from Halpha's own
    # central/core-component guesses (12, 6) to roughly match the ~1-2%
    # NII/Halpha flux ratio seen in the legacy fit
    # (output/NII/NII_Halpha_ratios.pkl).
    r_nii_ha = rest['[NII]6584'] / rest['Halpha']
    gaussians.append(build_tied('[NII]6584_A', 0, r_nii_ha, 0.3, (0, None)))  # 24
    gaussians.append(build_tied('[NII]6584_B_core', 2, r_nii_ha, 0.15, (0, None)))  # 25
    ...
    for name in line_order + ['[NII]6584']:
        ...
        ax[0].set_title(f"{name}  |  joint tied fit (B: red+central merged into 1 core)  |  z_A={z_A}  z_B={z_B}",
                        fontsize=12)
        ...
        fig.savefig(
            f'./output/joint_fit/twocomponent_B/{name}_joint_tied_fit.png')
        with open(
                f'./output/joint_fit/twocomponent_B/{name}_joint_tied_fit.pkl',
                'wb') as f:
            dill.dump({'model': bestfit_model, 'tie_map': tie_map}, f)

    print()
    print(f"Saved two-component-B joint fit outputs to ./output/joint_fit/twocomponent_B/")
