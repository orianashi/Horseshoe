...
lam_NIR = crval_NIR + cdelt_NIR * (
    bins_NIR + 1 - crpix_NIR)  # makes sure the array starts at the right place
...
lam_UVB = crval_UVB + cdelt_UVB * (
    bins_UVB + 1 - crpix_UVB)  # makes sure the array starts at the right place
...
lam_VIS = crval_VIS + cdelt_VIS * (
    bins_VIS + 1 - crpix_VIS)  # makes sure the array starts at the right place
...
def boxcar_smooth(flux, noise=None):
    ...
    noise_smooth = np.full_like(noise, np.nan,
                                dtype=float) if noise is not None else None
    noise_quad = np.full_like(noise, np.nan,
                              dtype=float) if noise is not None else None
    for i in range(len(flux)):
        if np.isnan(flux[i]
                    ):  # if ur nan then stay nan, works for large areas of nan
...
flux_uvb_smooth, noise_uvb_smooth, noise_uvb_quad = boxcar_smooth(
    flux_uvb_norm, noise_uvb_norm)
flux_vis_smooth, noise_vis_smooth, noise_vis_quad = boxcar_smooth(
    flux_vis_norm, noise_vis_norm)
flux_nir_smooth, noise_nir_smooth, noise_nir_quad = boxcar_smooth(
    flux_nir_norm, noise_nir_norm)
...
axes[0].fill_between(lam_uvb_rest,
                     flux_uvb_smooth - noise_uvb_plot,
                     flux_uvb_smooth + noise_uvb_plot,
                     alpha=0.2,
                     lw=1.5,
                     color='lightgrey',
...
axes[1].fill_between(lam_vis_rest,
                     flux_vis_smooth - noise_vis_plot,
                     flux_vis_smooth + noise_vis_plot,
                     alpha=0.2,
                     lw=1.5,
                     color='lightgrey',
...
axes[2].fill_between(lam_nir_rest,
                     flux_nir_smooth - noise_nir_plot,
                     flux_nir_smooth + noise_nir_plot,
                     alpha=0.2,
                     lw=1.5,
                     color='lightgrey',
...
for i in range(len(horseshoe_wl)):
    ...
    if (
            lam_uvb_rest[0] < rest_wl < lam_uvb_rest[-1]
    ):  #& (strength[i]>=np.median(strength))         # Only if within UVB wavelength range
        axes[0].axvline(rest_wl, color='red', ls='--', alpha=0.5, lw=0.8)
        axes[0].xaxis.set_minor_locator(
            MultipleLocator(10))  # draws vertical line
        axes[0].text(rest_wl,
                     1.01,
                     horseshoe_spec[i],
                     ...
                     fontsize=5.5,
                     ha='center',
                     color='red')  # places name of line at same wavelength
    if (lam_vis_rest[0] < rest_wl <
            lam_vis_rest[-1]):  # Only if within VIS wavelength range
        ...
        axes[1].text(rest_wl,
                     ...
                     ha='center',
                     color='red')
    if (lam_nir_rest[0] < rest_wl <
            lam_nir_rest[-1]):  # Only if within NIR wavelength range
...
for i in range(len(ism_wl)):
    ...
    if (lam_uvb_rest[0] < ism_gal_frame <
            lam_uvb_rest[-1]):  # Only if within UVB wavelength range
        axes[0].axvline(ism_gal_frame,
                        color='purple',
                        ls='--',
                        alpha=0.5,
                        lw=0.8)
        axes[0].xaxis.set_minor_locator(
            MultipleLocator(10))  # draws vertical line
        axes[0].text(ism_gal_frame,
                     1.01,
                     ism_spec[i],
                     transform=axes[0].get_xaxis_transform(),
                     fontsize=5.5,
                     ha='center',
                     color='purple')  # places name of line at same wavelength
    if (lam_vis_rest[0] < ism_gal_frame <
            lam_vis_rest[-1]):  # Only if within VIS wavelength range
        axes[1].axvline(ism_gal_frame,
                        color='purple',
                        ls='--',
                        alpha=0.5,
                        lw=0.8)
        axes[1].xaxis.set_minor_locator(MultipleLocator(10))
        axes[1].text(ism_gal_frame,
                     1.01,
                     ism_spec[i],
                     transform=axes[1].get_xaxis_transform(),
                     fontsize=5.5,
                     ha='center',
                     color='purple')
    if (lam_nir_rest[0] < ism_gal_frame <
            lam_nir_rest[-1]):  # Only if within NIR wavelength range
        axes[2].axvline(ism_gal_frame,
                        color='purple',
                        ls='--',
                        alpha=0.5,
                        lw=0.8)
        axes[2].xaxis.set_minor_locator(MultipleLocator(10))
        axes[2].text(ism_gal_frame,
                     axes[2].get_ylim()[1] * 1.01,
                     ism_spec[i],
                     transform=axes[2].get_xaxis_transform(),
                     fontsize=5.5,
                     ha='center',
