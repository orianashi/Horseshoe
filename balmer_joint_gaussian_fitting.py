...
# velocity dispersion between the tied component and its master)
# ==================
def create_mean_tie(ref_idx, line_ratio):

    def tie_mean(model):
        return getattr(model, f'mean_{ref_idx}') * line_ratio

    return tie_mean


def create_std_tie(ref_idx, line_ratio):

    def tie_std(model):
        return getattr(model, f'stddev_{ref_idx}') * line_ratio
...
def build_tied(label, ref_idx, line_ratio, amplitude_guess, amplitude_bound):
    g = models.Gaussian1D(name=label, mean=1.0, amplitude=amplitude_guess,
                          stddev=1.0)
    g.mean.tied = create_mean_tie(ref_idx, line_ratio)
    g.stddev.tied = create_std_tie(ref_idx, line_ratio)
...
for name in line_order:
    ...
    ax[0].set_title(f"{name}  |  joint tied fit  |  z_A={z_A}  z_B={z_B}",
                    fontsize=12)
    ...
    fig.savefig(
        f'./output/improved_gaussians/joint_fit/{name}_joint_tied_fit.png')
    with open(
            f'./output/improved_gaussians/joint_fit/{name}_joint_tied_fit.pkl',
            'wb') as f:
